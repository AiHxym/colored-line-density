 ...
 * @LastEditTime: 2023-02-27 15:19:11
    ...
    for (const [key, flattenBins] of Object.entries(binDensity)) {
        for (let flattenBin of flattenBins) {
            ...
            if (hues.length > 0 && hues[Math.floor(i / binSize) * Math.round(bins[i].length / binSize) + Math.floor(j / binSize)] !== undefined) {
                //console.log(i, j);
                binColor = chroma.hcl(hues[Math.floor(i / binSize) * Math.round(bins[i].length / binSize) + Math.floor(j / binSize)], Math.pow(0.3 + (1.2 - 0.3) * binDensityValue, 1) * 100, Math.pow(0.95 - (0.95 - 0.2) * binDensityValue, 1.5) * 100).hex();
            }
